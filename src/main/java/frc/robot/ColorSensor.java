...
public class ColorSensor
{
    ...
    public ColorSensor(I2C.Port port) {
        ...
        sensor = new I2C(port, 0x39); // 0x39 is the address of the Rev ColorSensor V2
        
        sensor.write(CMD | ENABLE_REGISTER, PON | AEN | PEN);
        sensor.write(CMD | ATIME_REGISTER, (int) (256-integrationTime/2.38)); //configures the integration time (time for updating color data)
    ...
    public int status() {
        boolean test;
        buffy.clear();
        test = sensor.read(STATUS_REGISTER, 1, buffy);
